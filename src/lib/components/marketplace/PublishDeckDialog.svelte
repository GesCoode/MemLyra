<script lang="ts">
  ...
  async function handleSubmit(event: SubmitEvent) {
    ...
    const deckTitle = result.deck?.title ?? title;
    const message = result.updated
      ? `Updated marketplace listing “${deckTitle}”.`
      : `Published “${deckTitle}” to the marketplace.`;
    ...
    {#if step === 'pick'}
      ...
      {#if publishableDecks.length === 0}
        ...
        <div class="confirm-dialog__actions">
          {#if onOpenAddCards}
            <button class="btn-secondary" type="button" onclick={onOpenAddCards}>Open add card</button>
          {/if}
          {#if onOpenImport}
            <button class="btn-secondary" type="button" onclick={onOpenImport}>Open import</button>
          {/if}
    ...
    {:else}
      ...
      <form class="publish-deck-dialog__form" onsubmit={handleSubmit}>
        ...
        <div class="confirm-dialog__actions">
          ...
          <button class="btn-publish" type="submit" disabled={submitting}>
